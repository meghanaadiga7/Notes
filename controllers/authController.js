...
exports.login = catchAsync(async (req, res, next) => {
	...
	if (!user || !correctPassword) return next(new AppError("Incorrect email or password", 401));
...
exports.protect = catchAsync(async (req, res, next) => {
	...
	if (!user) return next(new AppError("User was recently deleted.Please signup again", 401));
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
	...
	if (!user) return next(new AppError("No user found with that email ID", 401));
	const resetToken = user.createResetToken();
	await user.save({ validateBeforeSave: false });
	const resetURL = `${req.protocol}://${req.get(
		"host"
	)}/api/v1/notes/users/resetPassword/${resetToken}`;
...
exports.resetPassword = catchAsync(async (req, res, next) => {
	...
	if (!user) return next(new AppError("Invalid token or token has expired", 401));
...
exports.updateMyPassword = catchAsync(async (req, res, next) => {
	...
	if (!isPasswordCorrect) return next(new AppError("Your password is incorrect"), 401);
